...
export const setCartAsync = createAsyncThunk(
    'Cart/Update', async ({ UserId }: { UserId: string }, { dispatch }) => {
        const response = await axios.get(`${import.meta.env.VITE_PUBLIC_API_URL}/UserDashboard/${UserId}/Cart/Set`);
...
export const addToCartAsync = createAsyncThunk(
    "Cart/Add", async ({ UserId, Quantity, DeviceId }: { UserId: string, Quantity: number, DeviceId: string }, { dispatch }) => {
        const response = await axios.post(`${import.meta.env.VITE_PUBLIC_API_URL}/UserDashboard/${UserId}/Cart/Add`, { Quantity, DeviceId });
...
export const RemoveFromCartAsync = createAsyncThunk(
    "Cart/Remove", async ({ UserId, DeviceId }: { UserId: string, DeviceId: string }, { dispatch }) => {
        ...
        const response = await axios.post(`${import.meta.env.VITE_PUBLIC_API_URL}/UserDashboard/${UserId}/Cart/Remove`, { DeviceId });
        if (response && response.data) {
