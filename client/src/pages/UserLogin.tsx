...
export function UserLogin({ toggleForm }: UserLoginProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_PUBLIC_API_URL}/UserLogin`, data, {
  ...
  return (
    <>
      ...
      <form className="space-y-4 font-roboto" onSubmit={handleSubmit(onSubmit)}>
        ...
        <button
          type="button"
          onClick={() => window.location.href = `https://gizmo-sci1.onrender.com/auth/google`}
