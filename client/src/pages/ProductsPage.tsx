      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        {filteredDevices.length > 0 && (
          <div className="mt-10 flex justify-center items-center gap-4">
            <button
              onClick={() => {
                // First scroll to top, then update page
                window.scrollTo({ top: 0, behavior: "smooth" })
                // Use setTimeout to ensure scroll happens before state update
                setTimeout(() => {
            ...
            <div className="flex items-center gap-2">
              {Array.from({ length: Math.ceil(filteredDevices.length / devicesPerPage) }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => {
                    // First scroll to top, then update page
            ...
            <button
              onClick={() => {
                ...
                // Use setTimeout to ensure scroll happens before state update
                setTimeout(() => {
                  setCurrentPage((prev) => Math.min(prev + 1, Math.ceil(filteredDevices.length / devicesPerPage)))
                }, 200)
              }}
